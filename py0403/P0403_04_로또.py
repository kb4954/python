...
lotto2 = [i+1 for i in range(45)] # 로또 순차적번호, 출력을 하기위한 로또번호, 섞으면 안됨. 있는거에서 출력
    ...
    if choice == 1:
        ...
        while True:
            ...
            if choice<0 or choice>45:
                ...
                continue # 재실행
            temp = 0 # temp를 쓰지 않으면 print의 멘트가 45번 나옴
            for i in lotto2:
                if i==choice: 
                    lotto2[i-1] ="X" #lotto2에 들어있는 리스트는 0번방부터 시작하기 때문에 내가 입력한 숫자 i는 1부터 시작하므로 i-1이 되어야한다
                    my_lotto[count] = choice # append보다 속도가 빠름
                    count += 1 # 입력로또번호개수 1증가
                    temp = 1   # 로또번호 존재확인변수
            if temp == 0: 
                print(f"{choice}번 번호는 입력하신 번호입니다. 다시입력하세요. ")
        ...
        print(my_lotto.sort()) # sort() 정렬명령어
    elif choice == 0:
        print("[ 프로그램 종료 ]")
        break    
